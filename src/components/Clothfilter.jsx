...
const Clothfilter = () => {
    ...
    const searchQery = useSelector((state) => state.productData.search)
    ...
    useEffect(() => {
        axios.get('https://api.escuelajs.co/api/v1/products').then((response) => {
            const filteredProducts = response.data.filter((product) => product.category.name.toLowerCase() === 'clothes'.toLowerCase());
    ...
    const productSearch = () => {
        ...
        const product = products.filter((pro) => {
            ...
            const prodName = `${pro.title}`.toLowerCase()
